#!/usr/bin/env python
"""
Purpose: using pytest for a simple SYSTEM testing to interact with dockerized web servers within a CI/CD pipeline.
"""
import requests
...
def test_get_good_page():
    """
    Simulates the user navigating to the CRM webpage with an HTTP GET
    """
...
def test_get_bad_page():
    """
    Simulates a user navigating to an invalid URL with an HTTP GET
    """
    get_headers = {"Accept":"text/html"}
    for port in [5001, 5002]:
...
def _post_acct(acct):
    ...
    for port in [5001, 5002]:
        resp = requests.post(f"http://localhost:{port}", headers=post_headers, data=f"acctid={acct['acctid']}")
        assert resp.status_code == 200
